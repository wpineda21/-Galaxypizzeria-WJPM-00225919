...
void buscarElemento(vector<T> canciones, queue<T>* cola, string tit){
    ...
    cout << "Cancion no encontrada" << endl;
        ...
        cout <<"Entradas Ordenadas: ";
        for (int i = 0; i <= iter->comida; i++){
        if(iter->comida=BreadGarlic) {
        cout<<"Bread garlic 3.50$"<<endl;break;
        }else if(iter->comida=Pizzarolls){
        cout<<"pizzarolls 3.50$"<<endl;break;
        }else if(iter->comida=Cheesesticks){
        cout<<"cheeseticks 3.50$"<<endl;break;
        }
        }